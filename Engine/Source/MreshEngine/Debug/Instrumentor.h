...
#define ME_PROFILE 1
	...
	#define ME_PROFILE_BEGIN_SESSION(name, filepath) ::MreshEngine::Instrumentor::Get().BeginSession(name, filepath)
	#define ME_PROFILE_END_SESSION() ::MreshEngine::Instrumentor::Get().EndSession()
	#define ME_PROFILE_SCOPE(name) ::MreshEngine::InstrumentationTimer timer##__LINE__(name);
