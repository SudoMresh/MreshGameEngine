...
namespace MreshEngine
{
	...
	namespace Utils
	{
		...
		static GLenum MEFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
			case MreshEngine::FramebufferTextureFormat::RGBA8:
				return GL_RGBA8;
			case MreshEngine::FramebufferTextureFormat::RED_INTEGER:
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			ME_CORE_ASSERT(m_ColorAttachments.size() <= 4, "Should be 4 or less for now");
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			ME_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::MEFBTextureFormatToGL(specification.TextureFormat), GL_INT, &value);
	}
